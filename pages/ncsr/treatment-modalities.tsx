...
import {
  Loader2, ArrowLeft, Scissors, Pill, Radiation, Syringe,
...
export default function TreatmentModalitiesPage() {
  ...
  async function fetchRecords(pId: number) {
    setLoading(true);
    try {
      const { data } = await api.get(`/treatment-plans/${pId}/records`);
  ...
  async function deleteRecord(recordId: number) {
    try {
      ...
      setRecords((prev) => prev.filter((r) => r.treatmentRecordId !== recordId));
    ...
    <Layout>
      ...
      {records.length > 0 && (
        <div className="mb-6 space-y-2">
          ...
          {records.map((r) => (
            <div key={r.treatmentRecordId} className="flex items-center justify-between p-4 rounded-xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm">
              <div>
      ...
      <div className="max-w-2xl bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 space-y-5">
