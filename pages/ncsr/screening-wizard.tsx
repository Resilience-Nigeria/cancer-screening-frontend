...
// ----- Endpoints (adjust here if your routes differ) -----------------------
const ENDPOINTS = {
  createClient: () => `/clients`,
  updateClient: (clientId: string) => `/clients/${clientId}`,
  createVisit: (clientId: string) => `/clients/${clientId}/visits`,
  screening: (visitId: number, type: CancerType) =>
    `/visits/${visitId}/${type}-screening`,
...
// Sensible default method per module so the form starts valid-ish.
...
function defaultScreeningFor(ct: CancerType): Record<string, any> {
  ...
  if (DEFAULT_METHOD[ct]) base.method = DEFAULT_METHOD[ct];
  return base;
}

// Each module's endpoint validates its own field names. The wizard standardises
// on `screeningResult`, but the cervical / breast / colorectal endpoints expect
// `result` — so we send both, and the value is captured whichever the backend
// reads. Numeric fields are coerced to match the per-module request rules.
...
// Hide the per-module treatmentReferral field — it's captured once for all
// selected modules by a single shared control on the screening step.
...
function buildScreeningPayload(
  ...
  // shared treatment referral applies to every selected module
  if (referralValue) p.treatmentReferral = referralValue;
  // result <-> screeningResult alias
  if (p.screeningResult !== undefined && p.result === undefined) {
    p.result = p.screeningResult;
  }
  if (p.result !== undefined && p.screeningResult === undefined) {
    p.screeningResult = p.result;
  }
...
function GroupedForm({
  ...
  return (
    <div className="space-y-6">
      {groups.map((group, gi) => {
        const anyVisible = group.fields.some(
...
// Collect required (and required Yes/No) fields that are empty
...
export default function ScreeningWizardPage() {
  ...
  // Multiple modules can be screened in one encounter.
  ...
  // One screening form per selected module, keyed by cancer type.
  ...
  // One treatment-referral answer for the whole encounter (sent to every
  // screening endpoint and to the visit, which requires it).
  const [treatmentReferral, setTreatmentReferral] = useState<string>("not_referred");

  // Field-level validation highlight maps
  ...
  // screening errors keyed per cancer type
    ...
    (async () => {
      ...
      try {
        ...
        setBiodata((prev) => ({
          ...
          landmark: c.landmark ?? "",
          registrationDate:
            dateOnly(c.registrationDate ?? c.registration_date) || today(),
        }));
        ...
        setStepIndex(1); // skip biodata entry → pre-counselling
      } catch {
  ...
  // Load treatment-center facilities for the referral step
  useEffect(() => {
    (async () => {
      try {
        ...
        // referral list optional; ignore failure
      }
    })();
  }, []);
  ...
  // Derived overall result across all selected modules
  const positiveTypes = cancerTypes.filter(
    (ct) => screenings[ct]?.screeningResult === "positive"
  );
  const anyPositive = positiveTypes.length > 0;
  const anySuspicious = cancerTypes.some(
    (ct) => screenings[ct]?.screeningResult === "suspicious"
  );
  const overallResult = anyPositive
    ...
    ? "suspicious"
    : "negative";
  const outcomeCancerType = (positiveTypes[0] || cancerTypes[0] || "") as
    | CancerType
    | "";

  // Steps reshape: referral only when at least one module is positive
  const steps = useMemo(() => {
  ...
  // Liver requires viral status; otherwise risk groups are identical.
  const riskGroups = useMemo(
    () =>
      getRiskGroups(
        cancerTypes.includes("liver")
          ? "liver"
          : ((cancerTypes[0] as CancerType) || "breast")
      ),
    [cancerTypes]
  );
  ...
  function setScreeningField(ct: CancerType, name: string, value: any) {
    ...
    setScreeningErrors((prev) => {
      const ctErr = prev[ct];
      if (ctErr && ctErr[name]) {
        return { ...prev, [ct]: { ...ctErr, [name]: false } };
      }
      return prev;
    });
  }
  ...
  async function persistRisk(): Promise<boolean> {
    ...
    try {
      ...
      ["cigarettesPerDay", "smokingDuration", "alcoholUnitsPerWeek"].forEach(
  ...
  async function ensureVisit(): Promise<number | null> {
    ...
    try {
      ...
      if (!id) {
        toast.error(
          "The visit was created but its ID wasn't returned, so screenings can't be attached. Please try again."
        );
        return null;
      }
  ...
  async function persistScreening(): Promise<boolean> {
    ...
    // Validate every selected module first
    ...
    cancerTypes.forEach((ct) => {
      ...
      if (missing.length) {
        ...
        if (!firstMsg) {
          firstMsg = `${cancerLabel(ct)} — ${missing.map((m) => m.label).join(", ")}`;
        }
      }
    });
    ...
    try {
      ...
      if (!id) return false;
      // Post each module's screening to the shared visit
  ...
  async function persistOutcome(): Promise<boolean> {
    ...
    try {
      const screeningDate =
        (outcomeCancerType && screenings[outcomeCancerType]?.screeningDate) ||
        today();
      ...
      const payload: Record<string, any> = {
        ...
        linkageToTreatment:
          referral.facilityName || outcome.treatmentCommenced === "yes"
            ? "yes"
            : "no",
      };
  ...
  async function handleNext() {
    ...
    if (key === "referral" && !referral.facilityName) {
      toast.error("Select a referral hospital, or go back and adjust the result.");
      return;
    }
    if (key === "outcome") {
    ...
    <Layout>
        ...
        <div className="mt-4 rounded-3xl overflow-hidden bg-gradient-to-r from-green-900 via-green-800 to-green-700 shadow-xl">
          <div className="px-4 py-5 sm:px-8 sm:py-7 text-white">
            ...
            <p className="mt-2 text-xs sm:text-sm text-green-100">
              Screen for one or several cancers in a single encounter. Each step
              saves before moving on.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-3xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-lg p-4 sm:p-6">
          ...
          <div className="space-y-5">
            ...
            <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
              <Label className="sm:col-span-2">
                ...
                <Input
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.fullName ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
              ...
              <Label>
                ...
                <Select
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.gender ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
              ...
              <Label>
                ...
                <Input
                  type="date"
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.dateOfBirth ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
                  max={today()}
              ...
              <Label>
                ...
                <Input
                  type="tel"
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.phoneNumber ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
              ...
              <Label>
                ...
                <Select
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.screeningCategory ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
              ...
              <div className="sm:col-span-2 grid gap-4 grid-cols-1 sm:grid-cols-2">
                <Label>
                  ...
                  <Select
                    className={`mt-2 rounded-2xl h-12 ${
                      biodataErrors.stateOfOrigin ? "ring-2 ring-red-400 border-red-400" : ""
                    }`}
                ...
                <Label>
                  ...
                  <Select
                    className={`mt-2 rounded-2xl h-12 ${
                      biodataErrors.lgaOfOrigin ? "ring-2 ring-red-400 border-red-400" : ""
                    }`}
                  ...
                  >
                    ...
                    {biodata.stateOfOrigin
                      ...
                      : biodata.lgaOfOrigin && (
                          <option value={biodata.lgaOfOrigin}>
                            {biodata.lgaOfOrigin}
                          </option>
                        )}
                  </Select>
              ...
              <div className="sm:col-span-2 grid gap-4 grid-cols-1 sm:grid-cols-2">
                <Label>
                  ...
                  <Select
                    className={`mt-2 rounded-2xl h-12 ${
                      biodataErrors.stateOfResidence ? "ring-2 ring-red-400 border-red-400" : ""
                    }`}
                ...
                <Label>
                  ...
                  <Select
                    className={`mt-2 rounded-2xl h-12 ${
                      biodataErrors.lgaOfResidence ? "ring-2 ring-red-400 border-red-400" : ""
                    }`}
                  ...
                  >
                    ...
                    {biodata.stateOfResidence
                      ...
                      : biodata.lgaOfResidence && (
                          <option value={biodata.lgaOfResidence}>
                            {biodata.lgaOfResidence}
                          </option>
                        )}
                  </Select>
              ...
              <Label>
                ...
                <Input
                  type="date"
                  className={`mt-2 rounded-2xl h-12 ${
                    biodataErrors.registrationDate ? "ring-2 ring-red-400 border-red-400" : ""
                  }`}
        ...
        {currentKey === "cancer" && (
          <div className="space-y-5">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Choose one or more — options are filtered by the client&apos;s
              gender. Each selected module gets its own screening form on the next
              steps.
            </p>
          ...
          <div className="space-y-8">
            ...
            {cancerTypes.map((ct) => (
              ...
              >
                ...
                <GroupedForm
                  groups={withoutReferral(SCREENING_GROUPS[ct])}
            ...
            {/* One referral answer for every selected module + the visit */}
      ...
      <div className="mt-6 flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <Button
          ...
          onClick={handleNext}
          disabled={busy || (currentKey === "precounsel" && consentDenied)}
        >
          <span className="inline-flex items-center justify-center gap-2">
            {busy && <Loader2 className="w-4 h-4 animate-spin" />}
            {isLast ? "Finish & Save" : "Save & Continue"}
            {!isLast && !busy && <ChevronRight className="w-4 h-4" />}
          </span>
        </Button>
      </div>
    </Layout>
