...
export default function UsersManagementPage() {
  ...
  async function fetchFacilities() {
    try {
      const { data } = await api.get("/users/facilities");
      if (data.status) {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      const matchesFacility =
        filterFacility === "all" || user.facilityId.toString() === filterFacility;
      ...
      return matchesSearch && matchesFacility && matchesRole && matchesStatus;
    });
  }, [users, searchQuery, filterFacility, filterRole, filterStatus]);

  function openModal(mode: "view" | "add" | "edit", user?: User) {
    setModalMode(mode);
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filteredUsers.map((user) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      {user.facility?.facilityName || "N/A"}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-500">
                      {user.facility?.facilityCode}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
              ...
              {user.facility && (
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <p className="text-xs">{user.facility.facilityCode}</p>
                  </div>
                </div>
              )}
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="md:col-span-2">
                      ...
                      <p className="text-gray-900 dark:text-white">
                        {selectedUser.facility?.facilityName} (
                        {selectedUser.facility?.facilityCode})
                      </p>
                    </div>
                  </div>
                </div>
